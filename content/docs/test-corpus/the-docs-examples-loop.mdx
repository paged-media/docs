...
[Package anatomy](/docs/package-anatomy)). We could store each example as a finished
...
[The renderer](/docs/the-renderer) for what `paged-inspect` is introspecting.
...
[fidelity gate](/docs/test-corpus/how-we-test). The fidelity gate is pixel-deep but
...
job of the [fidelity loop](/docs/test-corpus/how-we-test), which runs over the
