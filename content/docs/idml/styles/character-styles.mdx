...
description: The CharacterStyle element and its attributes — identity, BasedOn, font and size, fill, tracking — the RootCharacterStyleGroup, and how a range that applies a character style can still override one of its attributes locally.
tier: pro
diataxis: reference
status: published
owner: paged-docs
...
[paragraph style](/docs/idml/styles/paragraph-styles) the run sits in. Every character
...
ultimately from the [paragraph style](/docs/idml/styles/paragraph-styles) the run sits
in. This page lists the attributes our parser reads off `<CharacterStyle>`; the
order they resolve in is covered under [conflict resolution](/docs/idml/styles/conflict-resolution).
...
[cascade](/docs/idml/styles/the-cascade) and the style a run applies when it wants
...
[conflict resolution](/docs/idml/styles/conflict-resolution). The run-level override
