---
title: The cascade
description: BasedOn links a style to a parent; the parser folds that chain into one resolved set of values, bounds cycles with a depth cap, and lets a local override on a range still win.
tier: intermediate
...
top of it is covered in [conflict resolution](/docs/idml/styles/conflict-resolution).
...
[conflict resolution](/docs/idml/styles/conflict-resolution). The
[local-override example](/docs/idml/styles/character-styles) there shows a range that
