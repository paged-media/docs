...
item. While [`GeometricBounds`](/docs/idml/geometry-coordinates/geometric-bounds)
...
- **`GeometryPathType`** — one contour (one closed or open loop). More than one
  of these makes a [compound path](/docs/idml/geometry-coordinates/compound-paths).
...
[`ItemTransform`](/docs/idml/geometry-coordinates/item-transform).
...
## Corner options render per variant
...
the full set of names, and the renderer now emits bespoke geometry for each one
— per corner, honouring the per-corner radius and the frame's join/miter/cap:
...
<AttrTable
  ...
  rows={[
    ...
    { attr: 'Inverse / Bevel', type: 'enum', support: 'supported', note: 'Concave quarter-circle (Inverse) and straight chamfer (Bevel), generated per corner.' },
    { attr: 'Inset / Fancy', type: 'enum', support: 'supported', note: 'Inset is a fold-in notch; Fancy is a three-arc scallop. Both emit real geometry (calibrated).' },
  ]}
/>

<SupportBadge status="supported" note="all five corner options emit per-corner geometry (Fancy = 3-arc scallop, Inset = fold-in notch); per-corner radii + join/miter/cap honoured" />
...
[`GeometricBounds`](/docs/idml/geometry-coordinates/geometric-bounds), which puts Y
