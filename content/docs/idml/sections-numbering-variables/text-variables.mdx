...
[stories & text → text variables](/docs/idml/stories-text/text-variables). This page
...
<SupportBadge feature="sections-numbering-variables.text-variables" note="known VariableTypes re-resolve against the current model/layout at render; ResultText is the fallback for the rest — paged-renderer/src/pipeline/links.rs" />
...
- **The page-number variable is a separate path.** The current- and next-page
  ...
  detailed in [stories & text → text variables](/docs/idml/stories-text/text-variables),
...
- **For extraction**, a `TextVariableInstance` contributes its `ResultText` like any
  other characters; see [extract all text](/docs/idml/stories-text/extract-all-text).
