...
<AttrTable
  ...
  rows={[
    { attr: 'ResultText', type: 'string', support: 'supported', note: 'The baked value from the last export; the renderer re-derives it for the variable kinds it knows, and keeps this as the fallback.' },
    { attr: 'AssociatedTextVariable', type: 'string ref (TextVariable/…)', support: 'supported', note: 'Points back at the definition; used to recompute the value for the supported VariableType kinds.' },
  ]}
/>

<SupportBadge status="supported" note="known VariableTypes (custom / file-name / page-count / dates / chapter-number / running-header) re-resolve at render; ResultText is the fallback for the rest — paged-renderer/src/pipeline/links.rs" />

`ResultText` is the baked snapshot — whatever InDesign computed the last time it
composed the document. For the variable kinds the renderer knows how to run, it
re-derives the value from the current model and layout instead of trusting that
snapshot: a "Running Header" instance picks up the live heading on its page, a
"File Name" instance reads the document name, and a date stamp re-formats from a
deterministic render clock. So a running header *does* update when you move the
paragraph it tracks. Only the kinds the renderer does not yet recompute fall back
to the baked text.
...
When you [extract all text](/docs/idml/stories-text/extract-all-text), a
