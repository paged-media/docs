...
description: The IDML MasterSpread element — how a body page points at a master, and how master items get stamped onto body pages at render time.
tier: intermediate
diataxis: reference
status: published
owner: paged-docs
...
reused verbatim to read it. Everything the [Spreads and pages](/docs/idml/layout-model/spreads-and-pages)
and [Page items and stacking](/docs/idml/layout-model/page-items-and-stacking) pages
...
mechanism described in [Override resolution](/docs/idml/layout-model/override-resolution).
...
master items display. The renderer honors it: a page that applies a master still
stamps its master items, but a spread that sets `ShowMasterItems="false"` hides
that master layer.
...
[Page items and stacking](/docs/idml/layout-model/page-items-and-stacking) page shows
...
**How are master items placed when the master has two pages?**
For a facing-pages master with a distinct left and right page, each master item is
routed to the body page whose ordinal matches the master page it belongs to. A
left-page rule lands on left body pages and a right-page rule on right body pages,
rather than both being stamped onto every page.
