...
same machinery covered in [frames & paths](/docs/idml/frames-paths). The run it nests
inside is the `CharacterStyleRange` from [stories & text](/docs/idml/stories-text); if
the `Story` → `ParagraphStyleRange` → `CharacterStyleRange` shape is not yet
familiar, read [story structure](/docs/idml/stories-text/story-structure) first — this
chapter assumes it.
